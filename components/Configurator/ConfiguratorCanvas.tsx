...
/* ---------- Stylized geometric car for configurator ---------- */
function ConfiguratorCar({ color = "#C6A769" }: { color: string }) {
...
/* ---------- Main ConfiguratorCanvas ---------- */
export default function ConfiguratorCanvas({ color = "#C6A769" }: ConfiguratorCanvasProps) {
  return (
    <div className="relative aspect-[4/3] bg-[#050505] overflow-hidden">
      <Suspense fallback={<LoadingPlaceholder />}>
        <Canvas
          camera={{ position: [4.5, 2, 3.5], fov: 38, near: 0.1, far: 100 }}
          shadows
          gl={{
            antialias: true,
        ...
        >
          ...
          <directionalLight position={[3, 2, 5]} intensity={0.3} color="#e8dcc8" />

          {/* Car */}
          <ConfiguratorCar color={color} />
          ...
          {/* User-controlled orbit — no auto-rotate */}
          <OrbitControls
            autoRotate={false}
            enablePan={false}
            enableZoom
